...
class PlatformInstaller:
    ...
    def check_python_version(self) -> bool:
        ...
        if version.major >= 3 and version.minor >= 8:
            self.log(f"✅ Python {version.major}.{version.minor}.{version.micro} meets requirements")
            return True
        else:
            self.log(f"❌ Python {version.major}.{version.minor}.{version.micro} does not meet requirements (3.8+)", "ERROR")
            return False
