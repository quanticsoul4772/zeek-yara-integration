...
from core.database import DatabaseManager, ConnectionPool, performance_track
...
class TestDatabasePerformance:
    ...
    def test_query_performance(self, db_manager, timer):
        ...
        for result in query_times:
            ...
            print(f"  All alerts query: {result['all_time']:.6f}s ({result['all_per_row']*1000:.2f}ms per row)")
            print(f"  Filtered query: {result['filter_time']:.6f}s ({result['filter_per_row']*1000:.2f}ms per row)")
