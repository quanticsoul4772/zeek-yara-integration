...
from core.scanner import SingleThreadScanner, MultiThreadScanner, FileEventHandler
...
class TestScannerIntegration:
    ...
    def test_eicar_detection(self, config):
        ...
        try:
            ...
            matching_alerts = [alert for alert in alerts 
                               if (alert.get('file_path') == eicar_path or 
                                   (eicar_name and alert.get('file_name') == eicar_name))]
...
class TestScannerPerformance:
    ...
    def test_single_vs_multi_thread_performance(self, config, timer):
        ...
        try:
            ...
            print(f"Speed comparison: Single-threaded vs. min multi-threaded: {single_time:.6f}s vs {min(thread_times.values()):.6f}s")
    ...
    def test_file_type_filtering_performance(self, config, timer):
        ...
        try:
            ...
            # Print results
            print(f"Unfiltered scan: {unfiltered_time:.6f}s for {unfiltered_result['scanned']} files")
            print(f"Filtered scan (text only): {filtered_time:.6f}s for {filtered_result['scanned']} files")
            
            # Calculate efficiency
            unfiltered_per_file = unfiltered_time / unfiltered_result['scanned']
            filtered_per_file = filtered_time / filtered_result['scanned'] if filtered_result['scanned'] > 0 else 0
    ...
    def test_rule_optimization_performance(self, config, timer):
        ...
        try:
            ...
            compile_speedup = standard_compile_time / optimized_compile_time if optimized_compile_time > 0 else 1
            scan_speedup = standard_scan_time / optimized_scan_time if optimized_scan_time > 0 else 1
