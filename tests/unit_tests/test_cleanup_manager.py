...
class TestFileCleanupManager(unittest.TestCase):
    ...
    def test_process_file_for_cleanup_old_unprocessed(self, mock_time):
        ...
        with patch.object(self.cleanup_manager.db_manager, 'get_alerts', return_value=[]):
    ...
    def test_process_file_for_cleanup_old_matched(self, mock_time):
        ...
        with patch.object(self.cleanup_manager.db_manager, 'get_alerts', return_value=[mock_alert]):
            result = self.cleanup_manager._process_file_for_cleanup(test_file)
    ...
    def test_check_disk_usage_alerts(self, mock_statvfs):
        ...
        with patch.object(self.cleanup_manager.logger, 'critical') as mock_critical, \
             patch.object(self.cleanup_manager.logger, 'warning') as mock_warning:
    ...
    def test_cleanup_processed_files_success(self, mock_time):
        ...
        # Mock database calls
        with patch.object(self.cleanup_manager.db_manager, 'get_alerts', return_value=[]), \
             patch.object(self.cleanup_manager, '_get_disk_usage', return_value={"usage_percent": 50.0}):
    ...
    def test_force_cleanup(self):
        ...
        with patch.object(self.cleanup_manager, 'cleanup_processed_files') as mock_cleanup:
