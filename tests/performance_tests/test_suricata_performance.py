    ...
    def benchmark(iterations=1):
        def decorator(func):
            def wrapper(*args, **kwargs):
                ...
                print(f"Benchmark: {func.__name__} - Total time: {total_time:.6f}s, Average: {total_time/iterations:.6f}s")
                return result
            return wrapper
        return decorator
...
def performance_test_env():
    ...
    with open(config_file, 'w') as f:
        f.write(f"""# Basic Suricata config for testing
%YAML 1.1
---
# Network Variables
vars:
...
class TestSuricataPerformance:
    ...
    def test_alert_retrieval_performance(self, performance_test_env, timer):
        ...
        assert duration < 5.0, f"Alert retrieval took {duration:.6f} seconds, which exceeds the 5-second threshold"
    ...
    def test_alert_filtering_performance(self, performance_test_env, timer):
        ...
        for filter_dict in filters:
            ...
            # Log performance metrics
            print(f"Filter {filter_dict}: Retrieved {len(filtered_alerts)} alerts in {duration:.6f} seconds")
            
            # Test should run in reasonable time
            assert duration < 1.0, f"Filtered retrieval took {duration:.6f} seconds, which exceeds the 1-second threshold"
    ...
    def test_correlation_performance(self, performance_test_env):
        ...
        for count in [10, 100, 500]:
            ...
            # Log performance metrics
            print(f"Correlated {alert_counts['yara_count']} YARA and {alert_counts['suricata_count']} Suricata alerts in {duration:.6f} seconds")
    ...
    def test_database_write_performance(self, performance_test_env, timer):
        ...
        # Test should run in reasonable time
        assert duration < 10.0, f"Correlation storage took {duration:.6f} seconds, which exceeds the 10-second threshold"
    ...
    def test_process_alerts_performance(self, performance_test_env, timer):
        ...
        # Test should run in reasonable time
        assert duration < 10.0, f"Alert processing took {duration:.6f} seconds, which exceeds the 10-second threshold"
