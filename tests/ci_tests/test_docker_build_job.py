...
class TestDockerBuildJob:
    ...
    def test_docker_buildx_setup_action(self):
        ...
        if result.returncode == 0:
            assert (
                "buildx" in result.stdout.lower()
            ), "Docker Buildx should be available"
        else:
            pytest.skip("Docker Buildx not available")
    ...
    def test_dockerfile_exists_check_logic(self):
        ...
        assert isinstance(
            expected_check, str
        ), "Dockerfile check should be a string command"
