...
class TestSecurityScanJob:
    ...
    def test_bandit_command_structure(self):
        ...
        assert '--recursive' in result.stdout or '-r' in result.stdout, "Bandit should support recursive scanning"
        assert '--format' in result.stdout or '-f' in result.stdout, "Bandit should support output format"
        ...
        try:
            ...
            result = subprocess.run(
                [
                    ...
                    '-f', 'json', '-o', 'temp_bandit_report.json'
                ], 
                capture_output=True, 
                text=True,
                cwd=project_root
            ...
            assert result.returncode != 0, "Bandit should detect security issues in test file"
    ...
    def test_bandit_project_scan(self):
        ...
        result = subprocess.run(
            [
                'bandit', '-r', '.', 
                '-x', '/venv,/extracted_files,/DATA/runtime',
                '-f', 'json', '-o', 'test_bandit_report.json'
            ], 
            capture_output=True, 
            text=True,
            cwd=project_root,
    ...
    def test_safety_dependency_check(self):
        ...
        if report_file.exists():
            try:
                with open(report_file, 'r') as f:
                    report_data = json.load(f)
                print(f"Safety report structure: {list(report_data.keys()) if isinstance(report_data, dict) else type(report_data)}")
            except json.JSONDecodeError:
    ...
    def test_security_report_upload_structure(self):
        ...
        for artifact in expected_artifacts:
            assert isinstance(artifact, str), f"Artifact name {artifact} should be string"
