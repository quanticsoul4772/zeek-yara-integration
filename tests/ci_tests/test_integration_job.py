...
class TestIntegrationTestJob:
    ...
    def test_integration_tests_directory_exists(self):
        ...
        assert (
            integration_tests_dir.exists()
        ), "Integration tests directory should exist"
        assert (
            integration_tests_dir.is_dir()
        ), "Integration tests path should be a directory"
    ...
    def test_zeek_installation_check(self):
        ...
        zeek_available = (
            subprocess.run(["which", "zeek"], capture_output=True).returncode == 0
        )

        if zeek_available:
            # Test zeek version if available
            result = subprocess.run(
                ["zeek", "--version"], capture_output=True, text=True
            )
    ...
    def test_yara_installation_check(self):
        ...
        yara_available = (
            subprocess.run(["which", "yara"], capture_output=True).returncode == 0
        )

        if yara_available:
            # Test yara version if available
            result = subprocess.run(
                ["yara", "--version"], capture_output=True, text=True
            )
    ...
    def test_suricata_installation_check(self):
        ...
        suricata_available = (
            subprocess.run(["which", "suricata"], capture_output=True).returncode == 0
        )

        if suricata_available:
            # Test suricata version if available
            result = subprocess.run(
                ["suricata", "--version"], capture_output=True, text=True
            )
    ...
    def test_configuration_file_copying(self):
        ...
        source_config = (
            project_root / "CONFIGURATION" / "defaults" / "default_config.json"
        )
    ...
    def test_pytest_xdist_available(self):
        ...
        assert (
            result.returncode == 0
        ), "pytest-xdist should be available for integration tests"
    ...
    def test_integration_tests_discoverable(self):
        ...
        assert (
            result.returncode == 0
        ), f"pytest should discover integration tests: {result.stderr}"
    ...
    def test_integration_tests_can_run(self):
        ...
        try:
            ...
            if result.returncode != 0:
                ...
                pytest.skip(
                    "Integration tests have dependency issues - expected for CI validation"
                )
    ...
    def test_job_dependency_validation(self):
        ...
        assert (
            dependency_order[1] == "integration-tests"
