...
class TestPerformanceBenchmarkJob:
    ...
    def test_performance_tests_directory_exists(self):
        ...
        assert performance_tests_dir.exists(), "Performance tests directory should exist"
    ...
    def test_benchmark_job_conditions(self):
        ...
        for condition in test_conditions:
            ...
            should_run = (condition['event'] == 'push' and 
                         condition['ref'] == 'refs/heads/main')
            
            assert should_run == condition['should_run'], \
    ...
    def test_benchmark_command_structure(self):
        ...
        assert any('--benchmark-json' in arg for arg in expected_command), "Should output JSON"
    ...
    def test_performance_tests_discoverable(self):
        ...
        result = subprocess.run(
            ['python', '-m', 'pytest', str(performance_tests_dir), '--collect-only', '-q'], 
            capture_output=True, 
            text=True,
            cwd=project_root
        )
        ...
        assert result.returncode in [0, 1], f"pytest should discover performance tests: {result.stderr}"
    ...
    def test_pythonpath_configuration_for_benchmarks(self):
        ...
        if platform_path.exists():
            ...
            assert result.returncode == 0, "Should be able to set PYTHONPATH for benchmarks"
    ...
    def test_benchmark_artifact_upload_structure(self):
        ...
        assert expected_artifact['name'] == 'benchmark-results', "Artifact name should be benchmark-results"
        assert expected_artifact['path'] == 'benchmark.json', "Artifact path should be benchmark.json"
    ...
    def test_benchmark_execution_simulation(self):
        ...
        if benchmark_file.exists():
            ...
            try:
                ...
                print(f"Benchmark data keys: {list(benchmark_data.keys()) if isinstance(benchmark_data, dict) else type(benchmark_data)}")
        ...
        if result.returncode != 0:
            pytest.skip("Performance benchmarks not working - this is expected for CI validation")
    ...
    def test_benchmark_marker_configuration(self):
        ...
        if pytest_ini.exists():
            ...
            if 'performance:' in pytest_config:
                ...
                print("Performance marker not found in pytest.ini - may need to be added")
        else:
    ...
    def test_main_branch_only_logic(self):
        ...
        for scenario, condition in github_conditions.items():
            # Replicate CI logic
            should_run = (condition['event_name'] == 'push' and 
                         condition['ref'] == 'refs/heads/main')
    ...
    def test_benchmark_performance_markers(self):
        ...
        for marker in marker_examples:
            assert marker.startswith('@pytest.mark.'), f"Marker {marker} should use pytest.mark"
