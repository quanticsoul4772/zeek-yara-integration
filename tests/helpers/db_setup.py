...
def initialize_test_db(db_file, seed_data=True, clear_existing=False):
    ...
    if seed_data:
        ...
        if yara_count == 0:
            # YARA Alert sample
            yara_alert = (
                timestamp, '/path/to/malicious.exe', 'malicious.exe', 1024,
                'application/x-dosexec', 'abcdef1234567890',
                '1234567890abcdefghijklmnopqrstuvwxyz1234567890abcdefghijklmn',
                'CqTvvd2Y19NiSsNN2J', 'MaliciousExe', 'malware',
                json.dumps({'severity': 3, 'description': 'Detected malicious executable'}),
                json.dumps(['string1', 'string2']), 3
            )
            c.execute('''
        ...
        if suricata_count == 0:
            ...
            suricata_alert = (
                timestamp, 'alert', '192.168.1.100', 45678, '10.0.0.10', 80, 'TCP', 'alert',
                1, 1000001, 1, 'TEST-1 HTTP Traffic', 'web', 2, 
