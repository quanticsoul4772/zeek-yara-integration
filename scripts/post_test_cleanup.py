...
def check_database_before_cleanup(db_path):
    ...
    try:
        ...
        cursor.execute("SELECT COUNT(*) FROM yara_alerts WHERE timestamp > datetime('now', '-1 hour')")
...
def run_full_cleanup(args):
    ...
    if args.verify_db:
        ...
        if not check_database_before_cleanup(db_path):
            if args.force:
                logger.warning("Database verification failed, but --force is enabled. Continuing with cleanup.")
            else:
                logger.error("Database verification failed. Cleanup aborted. Use --force to override.")
                return False
...
def parse_arguments():
    ...
    parser.add_argument('--verify-db', action='store_true', help='Verify database integrity before cleanup')
    parser.add_argument('--db-path', default='/Users/russellsmith/zeek_yara_integration/logs/yara_alerts.db', 
                        help='Path to the YARA alerts database')
    parser.add_argument('--force', action='store_true', help='Force cleanup even if verification fails')
    parser.add_argument('--delay', type=int, default=0, help='Delay in seconds before cleanup (default: 0)')
