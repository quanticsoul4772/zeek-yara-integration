...
class DistributedWorker:
    ...
    def setup_routes(self):
        ...
        async def receive_task(task_data: Dict[str, Any]):
            ...
            try:
                ...
                threading.Thread(target=self._process_task, args=(task_data,), daemon=True).start()
                
                return {"success": True, "message": "Task accepted", "worker_id": self.worker_id}
    ...
    def unregister_from_master(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.logger.warning(f"Failed to unregister from master: {response.status_code}")
                return False
    ...
    def start(self) -> bool:
        ...
        try:
            ...
            self.running = True
            self.logger.info(f"Worker {self.worker_id} started on port {self.worker_port}")
            
            # Start API server (blocking)
            uvicorn.run(self.app, host="0.0.0.0", port=self.worker_port, log_level="warning")
...
def main():
    ...
    parser.add_argument("--master-host", default="localhost", 
                       help="Master node host address")
    parser.add_argument("--master-port", type=int, default=8000, 
