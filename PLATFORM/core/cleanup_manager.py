...
class FileCleanupManager:
    ...
    def __init__(self, config: Dict):
        ...
        self.retention_matched = config.get("CLEANUP_RETENTION_MATCHED_HOURS", 168)  # 7 days
        
        # Cleanup schedule (in minutes)
        self.cleanup_interval = config.get("CLEANUP_INTERVAL_MINUTES", 60)
        
        # Disk usage thresholds
        self.disk_usage_warning_threshold = config.get("CLEANUP_DISK_WARNING_THRESHOLD", 80.0)
        self.disk_usage_critical_threshold = config.get("CLEANUP_DISK_CRITICAL_THRESHOLD", 90.0)
    ...
    def _get_files_for_cleanup(self) -> List[Path]:
        ...
        try:
            if not self.extract_dir.exists():
                self.logger.warning(f"Extract directory does not exist: {self.extract_dir}")
    ...
    def _get_file_processing_status(self, file_path: Path) -> Dict:
        ...
        try:
            ...
            alerts = self.db_manager.get_alerts(
                filters={"file_path": str(file_path.absolute())},
                limit=1
            )
