...
class MetricsCollector:
    ...
    def increment_counter(self, name: str, value: int = 1, tags: Optional[Dict[str, str]] = None) -> None:
        ...
        with self.lock:
            ...
            self._record_metric(name, MetricType.COUNTER, self.counters[name], tags or {})
...
class PerformanceTracker:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # Performance history
        self.performance_history: deque = deque(maxlen=1440)  # 24 hours of minute samples
        self.lock = threading.Lock()
        
        # Baseline metrics
        self.baseline_metrics: Dict[str, float] = {}
        self.baseline_calculation_samples = config.get("BASELINE_CALCULATION_SAMPLES", 100)
    ...
    def record_performance(self, metrics: Dict[str, Any]) -> None:
        ...
        performance_data = {
            ...
            "average_scan_time": metrics.get("scanner", {}).get("average_scan_time_ms", 0),
    ...
    def _update_baselines(self) -> None:
        ...
        try:
            recent_samples = list(self.performance_history)[-self.baseline_calculation_samples:]
            
            for metric in ["throughput", "error_rate", "average_scan_time", "cpu_percent", "memory_percent"]:
                values = [sample[metric] for sample in recent_samples if sample[metric] is not None]
                if values:
                    self.baseline_metrics[f"{metric}_baseline"] = statistics.median(values)
    ...
    def get_performance_trends(self, hours: int = 1) -> Dict[str, Any]:
        ...
        for metric in ["throughput", "error_rate", "average_scan_time", "queue_size", "cpu_percent", "memory_percent"]:
            values = [sample[metric] for sample in recent_data if sample[metric] is not None]
            ...
            if values:
                trends[metric] = {
                    ...
                    "baseline_deviation": self._calculate_baseline_deviation(metric, values[-1])
                }
    ...
    def _calculate_baseline_deviation(self, metric: str, current_value: float) -> Optional[float]:
...
class HealthChecker:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.health_thresholds = {
            ...
            "response_time_threshold": config.get("HEALTH_RESPONSE_TIME_THRESHOLD", 5.0),
            "queue_size_threshold": config.get("HEALTH_QUEUE_SIZE_THRESHOLD", 1000),
            "worker_availability_threshold": config.get("HEALTH_WORKER_AVAILABILITY_THRESHOLD", 0.8)
        }
