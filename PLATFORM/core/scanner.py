...
class BaseScanner:
    ...
    def __init__(self, config):
        ...
        self.yara_matcher = YaraMatcher(
            ...
            timeout=config.get('SCAN_TIMEOUT', 60)
        )
    ...
    def scan_file(self, file_path):
        ...
        if (self.config.get('SCAN_MIME_TYPES') and 
            not self.file_analyzer.filter_file_by_mime(
        ...
        if (self.config.get('SCAN_EXTENSIONS') and 
            not self.file_analyzer.filter_file_by_extension(
        ...
        if scan_result.get('matched', False):
            ...
            try:
                ...
                if add_result:
                    self.logger.info(f"Added alert to database for file: {file_metadata.get('name', '')}")
                else:
                    self.logger.error(f"Failed to add alert to database for file: {file_metadata.get('name', '')}")
            except Exception as e:
    ...
    def scan_directory(self, directory=None):
        ...
        try:
            ...
            for filename in os.listdir(directory):
                ...
                if scan_result.get('error') == 'Excluded MIME type' or \
                   scan_result.get('error') == 'Excluded extension':
                    results['skipped'] += 1
                else:
                    results['scanned'] += 1
        ...
        self.logger.info(f"Directory scan complete: {results['scanned']} files scanned, "
...
class MultiThreadScanner(BaseScanner):
    ...
    def start_monitoring(self):
        ...
        try:
            ...
            self.running = True
            self.logger.info(f"Started monitoring directory with {self.num_threads} threads: {self.extract_dir}")
