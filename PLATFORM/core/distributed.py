...
class MessageQueueManager(ABC):
    ...
    def __init__(self, config: Dict[str, Any]):
        self.config = config
        self.logger = logging.getLogger(f"zeek_yara.distributed.{self.__class__.__name__}")
    ...
    @abstractmethod
    def publish_task(self, task: Dict[str, Any], priority: int = TaskPriority.NORMAL.value) -> bool:
...
class RabbitMQMessageQueue(MessageQueueManager):
    ...
    def publish_task(self, task: Dict[str, Any], priority: int = TaskPriority.NORMAL.value) -> bool:
        ...
        try:
            ...
            self.logger.debug(f"Published task to RabbitMQ: {task.get('task_id', 'unknown')}")
            return True
        except Exception as e:
...
class KafkaMessageQueue(MessageQueueManager):
    ...
    def publish_task(self, task: Dict[str, Any], priority: int = TaskPriority.NORMAL.value) -> bool:
        ...
        try:
            ...
            self.logger.debug(f"Published task to Kafka: {task.get('task_id', 'unknown')}")
            return True
        except Exception as e:
...
class WorkerNode:
    ...
    def __init__(self, worker_id: str, host: str, port: int, capabilities: Optional[List[str]] = None):
    ...
    def update_heartbeat(self, task_count: int = None, processing_time: float = None) -> None:
    ...
    def can_accept_task(self) -> bool:
        ...
        return (
            self.status in [WorkerStatus.IDLE, WorkerStatus.HEALTHY] and
...
class LoadBalancer:
    ...
    def select_worker(self, workers: List[WorkerNode], task: Optional[Dict[str, Any]] = None) -> Optional[WorkerNode]:
        ...
        with self.lock:
            if self.algorithm == "load_based":
                ...
                self.logger.warning(f"Unknown load balance algorithm: {self.algorithm}, using load_based")
                return self._select_by_load(available_workers)
    ...
    def _select_by_capability(self, workers: List[WorkerNode], task: Optional[Dict[str, Any]]) -> WorkerNode:
        ...
        capable_workers = [
            w for w in workers 
            if required_caps.issubset(set(w.capabilities))
        ]
...
class WorkerNodeManager:
    ...
    def register_worker(self, worker_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            self.logger.info(f"Registered worker: {worker_id} at {worker.host}:{worker.port}")
            return True
        except Exception as e:
    ...
    def update_worker_heartbeat(self, worker_id: str, data: Optional[Dict[str, Any]] = None) -> bool:
        ...
        try:
            with self.lock:
                ...
                if data:
                    worker.total_processed = data.get("total_processed", worker.total_processed)
                    ...
                    for worker in self.workers.values():
                        ...
                        if time_since_heartbeat > self.heartbeat_timeout:
                            if worker.status != WorkerStatus.OFFLINE:
                                ...
                                self.logger.warning(f"Worker {worker.worker_id} marked as offline (no heartbeat for {time_since_heartbeat:.1f}s)")
                        elif time_since_heartbeat > self.heartbeat_timeout * 0.8:
                            if worker.status == WorkerStatus.HEALTHY:
                                worker.status = WorkerStatus.UNHEALTHY
                                self.logger.warning(f"Worker {worker.worker_id} marked as unhealthy")
...
class DistributedScanner:
    ...
    def _create_message_queue(self) -> MessageQueueManager:
        ...
        if queue_type == MessageQueueType.RABBITMQ.value:
            if not RABBITMQ_AVAILABLE:
                self.logger.warning("RabbitMQ requested but not available, falling back to in-memory")
            ...
            if not KAFKA_AVAILABLE:
                self.logger.warning("Kafka requested but not available, falling back to in-memory")
                return InMemoryMessageQueue(self.config)
    ...
    def start(self) -> bool:
        ...
        try:
            ...
            # Start task consumer
            self.stop_event.clear()
            self.task_consumer_thread = threading.Thread(target=self._task_consumer, daemon=True)
    ...
    def submit_scan_task(self, file_path: str, priority: int = TaskPriority.NORMAL.value, 
    ...
    def _task_consumer(self) -> None:
        ...
        def task_handler(task: Dict[str, Any]) -> None:
            try:
                ...
                if not worker:
                    self.logger.warning(f"No available workers for task {task.get('task_id', 'unknown')}")
                ...
                self.logger.info(f"Assigned task {task.get('task_id', 'unknown')} to worker {worker.worker_id}")
