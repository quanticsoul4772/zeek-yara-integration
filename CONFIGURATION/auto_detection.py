...
class SystemDetector:
    ...
    def _detect_system_paths(self) -> Dict[str, Any]:
        ...
        paths = {
            'home': Path.home(),
            'temp': Path('/tmp') if self.platform != 'windows' else Path(os.environ.get('TEMP', 'C:\\temp')),
    ...
    def _detect_zeek(self) -> Dict[str, Any]:
        ...
        if zeek_path:
            ...
            # Get version
            try:
                result = subprocess.run([zeek_path, '--version'], 
    ...
    def _detect_zeek_paths(self, zeek_path: str) -> Dict[str, Optional[str]]:
        ...
        try:
            ...
            result = subprocess.run([zeek_path, '--print-id'], 
    ...
    def _detect_zeek_capabilities(self, zeek_path: str) -> List[str]:
        ...
        try:
            ...
            result = subprocess.run([zeek_path, '--help'], 
                                  capture_output=True, text=True, timeout=5)
            if result.returncode == 0:
    ...
    def _detect_yara(self) -> Dict[str, Any]:
        ...
        if YARA_AVAILABLE:
            try:
                yara_info['version'] = yara.__version__ if hasattr(yara, '__version__') else 'unknown'
    ...
    def _detect_suricata(self) -> Dict[str, Any]:
        ...
        if suricata_path:
            ...
            try:
                result = subprocess.run([suricata_path, '--build-info'], 
                                      capture_output=True, text=True, timeout=10)
                if result.returncode == 0:
    ...
    def _detect_python_modules(self) -> Dict[str, Any]:
        ...
        required_modules = [
            'yara', 'watchdog', 'magic', 'fastapi', 'uvicorn', 
            'pydantic', 'sqlalchemy', 'requests', 'rich', 'netifaces'
        ]
    ...
    def _detect_system_tools(self) -> Dict[str, Any]:
        ...
        system_tools = [
            ...
            'sqlite3', 'netstat', 'ss', 'tcpdump', 'tshark'
        ]
    ...
    def _detect_network_configuration(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for interface in interfaces:
                ...
                if (network_info['default_interface'] is None and 
                    'ipv4' in iface_info['addresses'] and 
    ...
    def _generate_performance_configuration(self) -> Dict[str, Any]:
        ...
        config = {
            ...
            'MAX_MEMORY_MB': int(resources.get('recommended_memory_limit', 1024**3) / (1024**2)),
    ...
    def save_configuration(self, output_path: Optional[Path] = None) -> Path:
        ...
        if output_path is None:
            output_path = self.project_root / 'CONFIGURATION' / 'auto_detected_config.json'
    ...
    def validate_configuration(self, config: Optional[Dict] = None) -> Dict[str, Any]:
        ...
        if config.get('YARA_ENABLED') and not (
            tools.get('yara', {}).get('available') or 
            tools.get('yara', {}).get('python_module')
        ):
            validation_results['warnings'].append(
