...
def parse_args():
    ...
    parser.add_argument('--duration', type=int, default=0, help='Monitoring duration in seconds (0 = continuous)')
    parser.add_argument('--correlation-window', type=int, default=300, help='Alert correlation time window in seconds')
...
def main():
    ...
    try:
        ...
        if args.interface:
            ...
            if success:
                if args.duration > 0:
                    logger.info(f"Completed monitoring on {args.interface} for {args.duration} seconds")
                else:
            ...
            if success:
                ...
                if alerts:
                    ...
                    for signature, count in sorted(alert_counts.items(), key=lambda x: x[1], reverse=True)[:5]:
            ...
            if correlated_groups:
                ...
                for i, group in enumerate(correlated_groups[:5], 1):
                    ...
                    logger.info(f"  Primary alert: {group.get('primary_alert', {}).get('source', 'unknown')} - {group.get('primary_alert', {}).get('rule_name', 'unknown')}")
