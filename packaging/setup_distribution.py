...
setup_config = {
    ...
    'data_files': [
        ...
        ('share/zeek-yara-educational/education', [
            file for file in (PROJECT_ROOT / 'EDUCATION').rglob('*')
            if file.is_file() and file.suffix in ['.md', '.json', '.yaml']
        ] if (PROJECT_ROOT / 'EDUCATION').exists() else []),
    ],
    ...
    # Keywords for search
    'keywords': [
        'security', 'networking', 'education', 'zeek', 'yara', 'suricata',
        'network-monitoring', 'intrusion-detection', 'malware-analysis',
        'cybersecurity', 'tutorial', 'learning-platform', 'ids', 'nids'
    ],
