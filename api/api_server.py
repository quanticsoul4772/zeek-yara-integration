...
from core.scanner import SingleThreadScanner, MultiThreadScanner
...
class WebhookConfigModel(BaseModel):
    ...
    events: List[str] = Field(["alert"], description="Events to trigger webhook (alert, scan, startup, shutdown)")
...
from api.suricata_api import get_suricata_runner, get_alert_correlator
...
async def get_status(_: bool = Depends(verify_api_key)):
    ...
    if scanner:
        ...
        scanner_type = "Multi-threaded" if isinstance(scanner, MultiThreadScanner) else "Single-threaded"
    ...
    if os.path.exists(extract_dir) and os.path.isdir(extract_dir):
        extracted_files_count = len([f for f in os.listdir(extract_dir) if os.path.isfile(os.path.join(extract_dir, f))])
...
async def get_alerts(
    ...
    try:
        ...
        if rule_name:
            filtered_alerts = [a for a in filtered_alerts if rule_name.lower() in a.get('rule_name', '').lower()]
        
        if file_type:
            filtered_alerts = [a for a in filtered_alerts if file_type.lower() in a.get('file_type', '').lower()]
        ...
        if os.path.isfile(file_path):
            ...
            if scan_request.recursive:
                ...
                for alert in all_alerts:
                    ...
                    if alert_path.startswith(file_path) and alert_path.count('/') <= file_path.count('/') + 1:
                        # Parse JSON fields
                        rule_meta = alert.get('rule_meta', {})
                        if isinstance(rule_meta, str):
                            try:
