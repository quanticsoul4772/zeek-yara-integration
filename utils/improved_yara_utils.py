...
class ImprovedRuleManager:
    ...
    def reload_rules(self):
        ...
        with self._lock:
            ...
            rule_files = list(self.rules_dir.rglob("*.yar")) + list(
            ...
            self.logger.info(
                f"Loaded {len(self.compiled_rules)} rule sets in {elapsed:.2f}s"
            )
    ...
    def scan_file(
        self, file_path: str, timeout: int = 30, fast_mode: bool = False
    ) -> List[ScanResult]:
            ...
            if use_mmap:
                ...
                with open(file_path, "rb") as f:
                    with mmap.mmap(
                        ...
                        results = self._scan_data(
                            mmapped_file, str(file_path), timeout, fast_mode
                        )
            else:
    ...
    def _scan_data(
        self, data: bytes, file_path: str, timeout: int, fast_mode: bool
    ) -> List[ScanResult]:
        ...
        with self._lock:
            for namespace, rules in self.compiled_rules.items():
                try:
                    matches = rules.match(data=data, timeout=timeout, fast=fast_mode)
                    ...
                    for match in matches:
                        result = ScanResult(
                            ...
                            meta=match.meta,
                            strings=[
                                (s.offset, s.identifier, s.instances[0])
                    ...
                    self.logger.warning(
                        f"Scan timeout for {file_path} with {namespace}"
                    )
                except Exception as e:
