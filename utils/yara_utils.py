...
class RuleManager:
    ...
    def compile_rules(self, force=False):
        ...
        try:
            ...
            self.logger.info(f"Rule compilation completed in {time.time() - start_time:.2f} seconds")
            return True
                        ...
                        try:
                            for string_id, offset, string_data in match.strings:
                                match_info['strings'].append({
                                    'id': string_id,
                                    'offset': offset,
                                    'data': string_data.hex() if isinstance(string_data, bytes) else str(string_data)
                                })
                        except Exception as e:
